...
void main( uint3 GroupThreadID : SV_GroupThreadID, uint3 GroupID : SV_GroupID)
{
    ...
    p.direction = float4(reflect(direction[GroupThreadID.x], hash(addArr[GroupID.x].randomVector.xyz * 5.1 + float3(GroupThreadID.x * 2.3f, GroupThreadID.x * 1.4, GroupThreadID.x * 4.3))) * 1.f, addArr[GroupID.x].randomVector.w) * addArr[GroupID.x].initialDirection.w;
    p.direction += float4(addArr[GroupID.x].initialDirection.xyz, 0.0f);
	p.time.x = 0.0f;
