...
void main(uint3 DispatchThreadID : SV_DispatchThreadID)
{
	...
	p.position.xyz = emitterLocation.xyz + initialDirection.xyz * DispatchThreadID.x * 0.01f /*+ float3(0,(((DispatchThreadID.x%4)*60) + frameID)*0.02f, 0)*/;/*Use this for more randomness sin(DispatchThreadID.x*10)+cos(DispatchThreadID.x*6)+ direction[DispatchThreadID.x];*/
	p.position.w = 1.0f;
