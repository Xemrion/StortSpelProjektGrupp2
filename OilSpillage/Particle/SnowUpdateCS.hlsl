...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (myID < NumParticles.x && NumParticles.x > 0)
	{
		...
		float ySin = sin((sin(-5.25f * moveSine + 1) + p.position.y) * (1 / fieldSize) - (moveSine * 5.7f));
		...
		else
		{
			...
			p.velocity.xyz = 0/*(p.velocity.xyz + TimeFactors.x * float3(0, -1, 0)) / (1 + (friction * TimeFactors.x))*/;
			p.velocity.y = 0;
