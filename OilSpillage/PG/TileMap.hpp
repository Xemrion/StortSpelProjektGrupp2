...
public:
	...
	std::vector<short int> getNeighbouringIndices(V2u tilePosition) const noexcept;
	...
	inline Vector3 getWorldPosByIndex(int index)
	{
		return convertTilePositionToWorldPosition(getTilePosByIndex(index));
	}
	inline V2u getTilePosByIndex(int index)
	{
